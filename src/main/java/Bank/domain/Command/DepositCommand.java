...
public class DepositCommand implements Command {
    ...
    public void execute() {
        try {
            ...
            Category category = categoryService.findByNameAndType(categoryName, FlowDirection.INCOME);
            if (category == null) {
                category = categoryService.createCategory(categoryName, FlowDirection.INCOME);
            }

            Operation operation = operationService.deposit(account, category, amount, "Пополнение: " + categoryName);

            if (operation != null) {
                this.operationId = operation.getId().toString();
                System.out.println("Пополнено на " + amount + " RUB");
            }
